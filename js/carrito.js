...
function actualizarBotonCarrito() {
    ...
    let contenido = `<button type="button" class="btn btn-warning position-relative"><img src="images/basket.svg" alt="Carrito" width="32"><span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">${total_productos}</span></button>`;
    ...
    if (document.getElementById("productos_seleccionados")) {
        ...
        if (productos.length == 0) {
            ...
            for (const producto of productos) {
                ...
                <td class='text-center'><button class='btn btn-danger' onclick='eliminarProducto(${producto.id});' title='Eliminar Producto'>-</button> <b>${producto.cantidad}</b> <button class='btn btn-success' onclick='agregarProducto(${producto.id});' title='Agregar Producto'>+</button></td>
                <td class='text-center'><b>$${producto.precio * producto.cantidad}</b></td>                
                <td class='text-center'><b>USD ${parseFloat(producto.precio/cotizaionUSD).toFixed(2) * producto.cantidad}</b></td>
                <td class='text-end'><button class='btn btn-secondary' onclick='eliminarProducto(${producto.id});'><img src='images/trash.svg' alt='Eliminar' width='24'></button></td>
                </tr>`;
            ...
            <td class='text-center'><b>USD ${parseFloat(total_pagar/cotizaionUSD).toFixed(2)}</b></td>
            </tr>
            </table>`;
            productos_seleccionados.innerHTML = contenido;
        }
